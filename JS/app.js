...
(function () {
    ...
    /* =========================================================
       ...
       ---------------------------------------------------------
       A lightweight portfolio gateway. It is intentionally short,
       skippable, keyboard accessible, and never blocks the site
       permanently if JavaScript or animation is unavailable.
    ========================================================= */
    function initializeIdentityEntry() {
        ...
        let introSeen = false;
        try {
            introSeen = sessionStorage.getItem("sandeshIdentitySeen") === "1";
        } catch (error) {
            introSeen = false;
        }
        ...
        style.textContent = `
            .sb-entry {
                ...
                overflow: hidden;
                background:
                    radial-gradient(circle at 50% 45%, rgba(100,243,176,.075), transparent 34%),
                    radial-gradient(circle at 15% 85%, rgba(84,168,255,.045), transparent 30%),
                    #030608;
                color: #f1f5f9;
            ...
            .sb-entry::before {
                ...
                pointer-events: none;
                background-image:
                    linear-gradient(rgba(100,243,176,.035) 1px, transparent 1px),
                    linear-gradient(90deg, rgba(100,243,176,.035) 1px, transparent 1px);
            ...
            .sb-entry::after {
                ...
                height: 20%;
                pointer-events: none;
                background: linear-gradient(to bottom, transparent, rgba(100,243,176,.08), transparent);
                animation: sb-entry-scan 2.8s linear infinite;
            }
            .sb-entry.is-closing {
                opacity: 0;
                visibility: hidden;
            ...
            .sb-entry-card {
                ...
                border: 1px solid rgba(100,243,176,.22);
                border-radius: 20px;
                background: linear-gradient(145deg, rgba(12,20,27,.94), rgba(4,8,12,.96));
                box-shadow: 0 35px 100px rgba(0,0,0,.58), 0 0 70px rgba(100,243,176,.055);
                backdrop-filter: blur(18px);
                -webkit-backdrop-filter: blur(18px);
                animation: sb-entry-card-in .7s cubic-bezier(.2,.8,.2,1) both;
            }
            .sb-entry-top {
                display: flex;
                align-items: center;
            ...
            .sb-entry-title {
                margin-top: 12px;
                font: 700 clamp(2.4rem, 7vw, 5.5rem)/.95 "JetBrains Mono", monospace;
            ...
            .sb-entry-status span {
                ...
                color: #aeb9c5;
                text-align: center;
                font: 500 .55rem/1.35 "JetBrains Mono", monospace;
                letter-spacing: .06em;
            }
            .sb-entry-status b {
                display: block;
            ...
            .sb-entry-enter:hover,
            .sb-entry-enter:focus-visible {
                transform: translateY(-2px);
                box-shadow: 0 12px 35px rgba(100,243,176,.2);
            }
            .sb-entry-skip {
                border: 0;
            ...
            .sb-entry-skip:hover,
            .sb-entry-skip:focus-visible { color: #f1f5f9; }
            .sb-entry-foot {
                display: flex;
                justify-content: space-between;
                gap: 20px;
                ...
                color: #475569;
                font: 500 .55rem/1.5 "JetBrains Mono", monospace;
            }
            @keyframes sb-entry-card-in {
                from { opacity: 0; transform: translateY(18px) scale(.985); }
                to { opacity: 1; transform: none; }
            }
            @keyframes sb-entry-scan {
                from { transform: translateY(0); }
                to { transform: translateY(600%); }
            }
            @media (max-width: 620px) {
                .sb-entry { padding: 15px; }
                .sb-entry-card { padding: 25px 20px; border-radius: 16px; }
                .sb-entry-status { grid-template-columns: repeat(2, 1fr); }
                .sb-entry-actions { align-items: stretch; flex-direction: column; }
                .sb-entry-enter { width: 100%; }
                .sb-entry-skip { min-height: 40px; }
                .sb-entry-foot { flex-direction: column; gap: 6px; }
            }
            @media (prefers-reduced-motion: reduce) {
                .sb-entry, .sb-entry-card { animation: none !important; transition: none !important; }
                .sb-entry::after { display: none; }
            }
        `;
        ...
        entry.innerHTML = `
            <div class="sb-entry-card">
                <div class="sb-entry-top">
                    <span><i class="sb-entry-dot"></i>IDENTITY SYSTEM</span>
                    <span>SB / 01</span>
                </div>
                ...
                <div class="sb-entry-status" aria-label="Areas of focus">
                    <span><b>01</b>NETWORKING</span>
                    <span><b>02</b>CYBERSECURITY</span>
                    <span><b>03</b>DEVELOPMENT</span>
                    <span><b>04</b>AI / SYSTEMS</span>
                </div>
                <div class="sb-entry-actions">
                    <button class="sb-entry-enter" type="button">ENTER PORTFOLIO&nbsp; →</button>
                    <button class="sb-entry-skip" type="button">SKIP INTRO</button>
                </div>
                <div class="sb-entry-foot">
        ...
        const closeEntry = function () {
            ...
            try {
                sessionStorage.setItem("sandeshIdentitySeen", "1");
            } catch (error) {
